...
const App = () => {
  ...
  const handleLoadMore = async () => {
    ...
    let pageToken =
      localPlaylist[localPlaylist.length - 1].nextPageToken ?? null
    let channelId =
      localPlaylist[localPlaylist.length - 1].items[0].snippet.channelId ?? null
    let channelName =
    ...
    let nextPlaylistSet = await getChannelPlaylist(
      channelId,
      FormData.api_key,
      pageToken,
    )
  ...
  const handleAddChannel = () => {
    ...
    setPlaylistData(
      createPlaylistData({ [channelId]: localPlaylist }, playlistData),
    )
  ...
  const handleReloadChannelPlaylist = async () => {
    ...
    let localPlaylist_lowestVisible_entry =
      localPlaylist[Math.floor(videocard_lowestVisible_index / 50)].items[
        videocard_lowestVisible_index % 50
      ]
    let localPlaylist_lowestVisible_entry_id =
      localPlaylist_lowestVisible_entry.id
    ...
    for (
      let i_playlist = 0;
      !newPlaylist_lowestVisibleFound ||
      newPlaylists.length < localPlaylist.length;
      i_playlist++
    ) {
      //fetch playlist
      ...
      let newPlaylist =
        ...
        : await getChannelPlaylist(
            channel_id,
            FormData.api_key,
            newPlaylists[newPlaylists.length - 1].nextPageToken,
          )
      ...
      for (let i_video = 0; i_video < newPlaylist.items.length; i_video++) {
        ...
        //for the main loop
        if (
          newPlaylist.items[i_video].id === localPlaylist_lowestVisible_entry_id
        ) {
    ...
    let videocard_height =
      videocard.clientHeight +
      parseFloat(
        window.getComputedStyle(videocard).getPropertyValue("margin-top"),
      )
  ...
  const handleChannelListItemRemove = (channelId) => {
    ...
    setPlaylistData(createPlaylistData(undefined, newPlaylistData))
    setChannelData(createChannelData(undefined, newChannelData))
    setScrollData(createScrollData(undefined, newScrollData))
  }
    ...
    <>
      <section className="controls_section">
        ...
        <div className="button_row">
          ...
          <button className="button" onMouseDown={handleToggleChannelListView}>
            <svg viewBox="0 0 48 48">
              <use
                href={
                  showChannelList ?
                    `${icon_map}#i_chevron_up`
                  : `${icon_map}#i_chevron`
                }></use>
            </svg>
            ...
            for (const channelId in channelData) {
              result.push(
                <ChannelListItem
                  ...
                  clickHandler={() => {
                    handleChannelListItemClick(channelId)
                  }}
                  removeHandler={() => {
                    handleChannelListItemRemove(channelId)
                  }}
                />,
              )
            }
        ...
        <div>
          ...
          {localPlaylist.length > 0 ?
            <button
              ...
              disabled={canLoadMore ? false : true}
              className="button videolist_button marginTop"
              content="Load More">
              Load More
            </button>
          : null}
      ...
      <button className="button button_floating" onClick={handleScrollToTop}>
        Scroll to Top
      </button>
      {toast !== null ?
        <Toast
